...
//	[HideInInspector]
	...
	[HideInInspector]
	public float borderPixelOffset = 0.25f;
	[HideInInspector]
...
//				UpdateAutoTile( x, y );
				...
				uvRight *= (tileSize-(2f*borderPixelOffset))/tileSize;
				uvTop *= (tileSize-(2f*borderPixelOffset))/tileSize;
...
//		if( worldPos.y + 1 > ( transform.position.y + mapHeight ) * zoomFactor ) {
	...
	public int GetWorldAutoTileIndex ( int localX, int localY ) {
		if( localX < 0 ) {
			if( localY < 0 ) {
				int index = downLeftLayer == null ? -1 : downLeftLayer.WorldPointToMapIndex( (Vector2)transform.position + new Vector2( localX+0.5f, localY+0.5f ) * zoomFactor );
				return index == -1 ? -1 : downLeftLayer.tiles[ index ].autoTileIndex;
			}
			else if( localY >= mapHeight ) {
				int index = upLeftLayer == null ? -1 : upLeftLayer.WorldPointToMapIndex( (Vector2)transform.position + new Vector2( localX+0.5f, localY+0.5f ) * zoomFactor );
				return index == -1 ? -1 : upLeftLayer.tiles[ index ].autoTileIndex;
			}
			else {
				int index = leftLayer == null ? -1 : leftLayer.WorldPointToMapIndex( (Vector2)transform.position + new Vector2( localX+0.5f, localY+0.5f ) * zoomFactor );
				return index == -1 ? -1 : leftLayer.tiles[ index ].autoTileIndex;
			}
		}
		else if( localX >= mapWidth ) {
			if( localY < 0 ) {
				int index = downRightLayer == null ? -1 : downRightLayer.WorldPointToMapIndex( (Vector2)transform.position + new Vector2( localX+0.5f, localY+0.5f ) * zoomFactor );
				return index == -1 ? -1 : downRightLayer.tiles[ index ].autoTileIndex;
			}
			else if( localY >= mapHeight ) {
				int index = upRightLayer == null ? -1 : upRightLayer.WorldPointToMapIndex( (Vector2)transform.position + new Vector2( localX+0.5f, localY+0.5f ) * zoomFactor );
				return index == -1 ? -1 : upRightLayer.tiles[ index ].autoTileIndex;
			}
			else {
				int index = rightLayer == null ? -1 : rightLayer.WorldPointToMapIndex( (Vector2)transform.position + new Vector2( localX+0.5f, localY+0.5f ) * zoomFactor );
				return index == -1 ? -1 : rightLayer.tiles[ index ].autoTileIndex;
			}
		}
		else if( localY < 0 ) {
			int index = downLayer == null ? -1 : downLayer.WorldPointToMapIndex( (Vector2)transform.position + new Vector2( localX+0.5f, localY+0.5f ) * zoomFactor );
			return index == -1 ? -1 : downLayer.tiles[ index ].autoTileIndex;
		}
		else if( localY >= mapHeight ) {
			int index = upLayer == null ? -1 : upLayer.WorldPointToMapIndex( (Vector2)transform.position + new Vector2( localX+0.5f, localY+0.5f ) * zoomFactor );
...
public class Tile {
	...
	public Vector2 GetBottomLeftUV ( TileInfo tileInfo ) {
		...
		result.x += tileInfo.borderPixelOffset/mat.mainTexture.width;
		result.y += tileInfo.borderPixelOffset/mat.mainTexture.height;
